...
| 항목 | 값 |
|---|---|
| 대상 | `RASBERY_GPU_PPR` arm의 Picard 종료 판정, 입력 업로드, per-slot 할당 |
| 상위 계획 | `docs/GPU_RASBERY_BOTTLENECK_PARALLEL_ACCELERATION_IMPLEMENTATION_PLAN_20260830_KO.md` §WP6 단계 B·C·E |
| 선행 | 단계 A 완료 (238): `PPR_GPU` ON×2 결정론, Gate A **0.0000 / 35 statepoint 전부**, receipt `host_fallbacks 0` · `iterations 816` · `host_iterations 0` |
| 게이트 등급 | 단계 B **B0 (현 device arm 대비)** — fold의 association이 host fold와 동일 / 단계 C **B0 조건부**, 조건은 borrow의 건전성이고 그것은 `verify` 모드가 측정한다 / 전체는 host arm 대비 여전히 **N1**(device `exp`, chunked corner sum — `c502856`이 건너간 선) |
| 플래그 | `RASBERY_GPU_PPR=1`(기본 0) 안에서 `RASBERY_GPU_PPR_DEVICE_LOOP`(기본 **1**), `RASBERY_GPU_PPR_GRAPH`(기본 0), `RASBERY_GPU_PPR_CANONICAL`(기본 `off`; `1`=borrow, `verify`=borrow+대조) |
| receipt | `[RASBERY][PPR_GPU]` **schema_version 2** — `loop_arm`, `host_syncs`, `host_syncs_per_statepoint`, `graph_launches`, `graph_builds`, `graph_refusal`, `canonical_mode`, `canonical_statepoints`, `canonical_mismatch`, `h2d_bytes`, `h2d_bytes_elided`, `d2h_bytes`, `allocations`, `reallocations` |
| 계약 테스트 | `tools/test_ppr_gpu_contract.py` — 20 property, **negative control 22종** |
| 소스 | `src/CudaPprBackend.{h,cu}`, `src/CudaPprBackendStub.cpp`, `src/PPR.{h,cpp}`, `src/Driver.h`(PPR 블록·receipt) |
| 기준 덱 | KNGR, `nxyz = 8,451`, `nsurf = 26,692`, `NG = 2`, 집합체 88(쿼터), 핀 16×16 |
| 로컬에서 한 것 | 순수 python 계약(20 property + 음성대조 22), MSVC `/Zs`로 `src/PPR.cpp`·`src/CudaPprBackendStub.cpp`·`Driver.h`를 include하는 TU, 그리고 `CudaPprBackend.cu`를 `tools/check_cuda_syntax.py`의 재작성 규칙 + graph/conditional shim으로 **`CUDART_VERSION` 12060·13000 양쪽** 문법 검사. **nvcc는 없다 — 238 컴파일이 첫 관문이다.** |
...
§6.4의 wall이 답한다 — `device_graph`가 존재하는 이유가 그 launch 900개이고,
...
## 5. 계약 테스트 — 20 property, 음성대조 22

`tools/test_ppr_gpu_contract.py`. property 1–10은 `c502856`의 것 그대로,
11–20이 WP6이다. **음성대조가 이번에 추가되었다**: 각 항목이 소스를 한 군데 망가뜨리고
...
### 6.1 단일 production arm — B0 게이트가 먼저다
...
1. `h5diff -c a.h5 b1.h5` 에서 **pin 데이터셋을 제외한 모든 데이터셋이 0 차이**
   (pin power / pin flux / Fq / FdH 는 N1이므로 §6.2에서 따로 본다)
...
5. `[RASBERY][PPR_GPU]` 의 `reallocations == 0`, `allocations == 26`

1~5 중 하나라도 어긋나면 **여기서 멈춘다.** wall은 재지 않는다.

### 6.2 Gate A — pin / Fq / FdH (NaN-aware)
...
### 6.4 단계 C — borrow의 건전성이 먼저, 바이트는 그 다음
...
기준 **878 c/h**(WP4 최고 arm) 대비. 채택 문턱 **+3 %**(§6.4 노이즈 규칙).
동시에 확인:
...
2. **`d2h_bytes`는 줄지 않았다.** 계수 7배열 9,059,472 B / statepoint가 그대로 내려온다.
   그것은 host `reconstructPinPower`가 `_p/_a/_c/_bt`를 읽기 때문이고, 계획 §WP6 단계 D가
   그 소비자를 device로 옮기면 함께 사라진다. **단계 D는 이 커밋에 없다.**
3. **`device_stream`은 launch를 900개로 늘린다.** `device_graph`가 그것을 1개로 만들지만
   capture 주장을 하나 더 얹는다. 어느 쪽이 이득인지는 §6.3/§6.5의 실측이 정한다.
4. **nvcc가 이 파일을 본 적이 없다.** 로컬 문법 검사는 shim 위에서 한 것이고,
