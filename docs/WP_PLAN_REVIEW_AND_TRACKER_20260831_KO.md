...
### 5.3 238 PROD 가격 평가 결과 (2026-08-30) — 네 기능이 한꺼번에 값을 얻었다

전문은 **`docs/PRICING_PROD_20260830_KO.md`**, 후보 arm은
**`test/reference/validation_baseline_manifest_v4_candidate.json`**.
...
| **v4 전체 스택 (블록 9)** | — | 위 넷 + `_PPR_GRAPH=1` | N1 (CRAM 지배) | **16.658** | **14.378** | **−2.280 (−13.7 %)** | `1f36e75dc00ed2b4` / 4377 | **v4 후보 · MASTER 27.2 s 대비 1.89×** · (e) vs (d) h5diff **0** |

**v4 후보 env** = PROD + `RASBERY_GPU_CRAM=1 RASBERY_GPU_CMFD_FUSE=15 RASBERY_GPU_PPR=1
...
### 5.4 다음 레버 — 이 캠페인이 손댄 것은 하나도 남지 않았다

**1차 셋** (블록 9b가 지목):

| # | 레버 | 근거 | 계기 | 크기 |
|---|---|---|---|---|
| **1** | **`result_write` 비동기화 — WP12(진행 중)** | host I/O가 `total_seconds`의 **19.9 %(2.894 s)**, `io_wall`은 사실상 전부 `result_write`. **CRAM+search+TH+PPR+Xe를 합친 것보다 크다** | SPTELEM | **대** |
...
| **3** | **`kernelFlatXs` (xsrecon/flatxs)** | **GPU 커널 시간 58.9 %(950.4 ms)** — Xe/TXN 20.8 % + CRAM 14.8 %를 합친 것보다 크다. host 타이머로는 13.2 %로만 보인다. **WP5의 CTA-per-node 커널이 이미 트리에 있고 기본값 OFF·미가격** | **nsys** | **대** |

**#2와 #3은 서로 어긋나는 두 계기가 지목한 것이다(§5.3).** nsys는 CMFD/outer + nodal/PPR device
커널을 합쳐 6 % 미만으로 본다 — 그렇다면 34 %는 CMFD 연산이 아니라 host 대기다. **손대기 전에
untelemetered 실행의 matched trace가 필요하다.**

**부차:**

| # | 후보 | 근거 | 크기 |
|---|---|---|---|
| 4 | **PCIe 전송 40.3 GB / ~110 K copy** | D2H 28.6 GB(55,183) + H2D 11.7 GB(54,918), 단일덱 1회. D2H가 copy 시간의 67.8 %. `cudaMemcpyAsync` 110,078 호출 | 중 |
