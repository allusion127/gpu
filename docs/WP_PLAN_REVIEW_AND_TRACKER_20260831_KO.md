...
| 갱신 | 2026-08-30 KST(238 PROD 가격 평가 도착) — **WP6-F·Task 16·WP7-B·WP7-C 네 기능이 전부 PROD env에서 가격을 얻었다.** 스택(`CRAM=1`+`FUSE=15`+`PPR=1`) **14.631 s = MASTER 27.2 s 대비 1.86×**(같은 커밋 base 16.585 s에서 −1.954 s, −11.8 %). PPR은 `91004f7` 이전까지 production에서 **한 번도 돌지 않았다**(`host_fallbacks:35/35`, `src/PPR.cpp:79`). 손 pin `RASBERY_XE_FORMS=0xadd`는 **181 전용 아티팩트로 폐기**했다. 새 문서 `docs/PRICING_PROD_20260830_KO.md`, 후보 매니페스트 `test/reference/validation_baseline_manifest_v4_candidate.json`. §5.3·§5.4 참조. |
...
| **스택** | — | 위 셋 동시 | N1 (CRAM 지배) | 16.585 | **14.631** | **−1.954 (−11.8 %)** | `1f36e75dc00ed2b4` / 4377 | **v4 후보 · MASTER 27.2 s 대비 1.86×** |

**v4 후보 env** = PROD + `RASBERY_GPU_CRAM=1 RASBERY_GPU_CMFD_FUSE=15 RASBERY_GPU_PPR=1
RASBERY_GPU_PPR_GRAPH=1 RASBERY_GPU_XE_TXN=1`.
**주의: 이 env 전체가 한 arm으로 측정된 적은 없다.** 14.631 s는 `CRAM+FUSE=15+PPR`까지의 스택이고
`_PPR_GRAPH`·`XE_TXN`은 따로 가격이 매겨졌다(GRAPH −0.18 s, TXN B0·중립). 원천 보고서의 블록 9가
바로 그것을 재기로 되어 있었고 **미완**이다. 후보 매니페스트가 이 사실을 `measured_arm_note`에 적는다.
...
### 5.4 다음 병목 후보 — 남은 14.6 s는 outer 루프다

| # | 후보 | 근거 | 크기 |
|---|---|---|---|
| **1** | **CRAM BOS micx residency** | `[CRAM_GPU].bos_reuse:34` = statepoint 수. beginning-of-step 버퍼가 resident로 남지 않고 statepoint마다 **~40.2 MB**를 재전송한다(총 `micx_h2d_mb:1367.9`). CRAM `wall_ms` 234 ms 안의 전송분이 상금이다 | 중 |
| **2** | **outer/CMFD 루프 — 지금의 지배항** | 14.631 s / **4377 outer = ~3.3 ms/outer**. PPR device 루프는 이제 **106 ms**로 전체의 0.7 %다 — 즉 **PPR은 더 이상 후보가 아니다.** 남은 wall은 outer 루프가 전부다 | **대(지배)** |
| **3** | **A2 — outer 수 감소** | #2의 직접적 귀결. 커널을 더 빠르게 하는 것보다 **outer를 덜 도는 것**이 이제 더 큰 레버다. `docs/A2_OUTER_REDUCTION_20260829_KO.md`(승수 스캔 14 arm, outer −61.6 %) | **대** |
| **4** | **배치 tail / `width_fill`** | 8×M8+MPS의 `width_fill` 0.41. 단일 wall이 내려갈수록 배치 tail의 상대 지분이 오른다. 기준 878 c/h | 중(배치 축) |
| **5** | `7cfe3a4`가 이웃보다 0.4 s 빠른 것 | 블록 1: `7cfe3a4` 16.373 vs `926497d`/`388e8f2`/`73f8627` 16.76–16.79. `388e8f2`가 후자 쪽에 앉으므로 실재한다면 `926497d`↔`7cfe3a4` 사이다. 다만 0.4 s(~2.4 %)는 세션 전 블록에서 관측된 통상 jitter와 같은 크기 | **낮음** |
