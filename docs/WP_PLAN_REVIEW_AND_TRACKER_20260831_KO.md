...
| **WP8** | 장수명 GA evaluator | **단계 1 코드·계약 done, 미가격** (`1cdd724` `--evaluator-jsonl` wave 프로토콜 + `Driver::CaseReceipt`, `a51835f` 5부 계약 테스트(음성 대조군 12개, 프로세스 수명 가변 상태 22건 분류) + `tools/evaluator_client.py` 3-arm 하네스, `docs/WP8_EVALUATOR_STAGE1_20260831_KO.md`) / **단계 2(CohortContext)·3(buffer reuse) not-started** | 본 작업 | **238: 64케이스 × 3세대 연속 1프로세스 vs 3프로세스.** B0가 먼저 — persistent와 per-generation의 케이스별 `[EVALUATOR][CASE].digest` 192/192 동일, 세대 간 같은 덱 digest 동일, `xslib_loads == 1`, `slot_{duplicates,stale_tenants,double_releases}` = `pin_live_ranges_between_waves` = `isolation_mismatches` = 0. 그 다음에야 c/h: stand-up 비용 ≤1% 전체 wall, chunked-jobs 대비 ≥+5%. 미달이면 "정확성 계약과 운영 편의만 얻었다"로 기록한다 | WP1+WP2 안정화. **238 빌드만 남았다**(로컬은 문법 검증 + 순수 파이썬 계약뿐). 잔여 구멍: `run_multi_gpu_batch.py`는 미개조(청크마다 `subprocess.run`이라 지속 `Popen` 프로토콜이 필요 — 단계 1.5 TODO), worker 교체(`fatal=true`) 없음, XSLIB 캐시 키는 여전히 `(path,size,mtime,ng)`(단계 2의 `CohortKey`에서 content digest와 함께 한 번에) |
