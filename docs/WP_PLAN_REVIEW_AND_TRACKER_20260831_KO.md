...
| **WP6** | GPU PPR device loop/reconstruct/canonical | **단계 A done(238, 미가격)** — `PPR_GPU` ON×2 결정론, Gate A **0.0000 / 35 statepoint 전부**, `host_fallbacks 0` · `iterations 816` · `host_iterations 0` / **단계 B·C·E 코드·계약 done, 미가격** — 본 커밋: Picard 종료 판정이 device로 갔다 (`kCornerFoldAndCheck<<<1,4>>>`가 **host와 같은 association**으로 chunk 0→255를 접고 같은 `RelativeChange`·`1e-5`·네 corner 논리곱을 적용한다), 라운드마다의 `cudaStreamSynchronize`가 **statepoint당 24.31 → 1**; arm 3종 (`host_sync`=`c502856` 대조 / `device_stream` 기본 — 남는 body는 **전 커널이 flag를 읽고 쓰기 전에 반환**하므로 빈 라운드이고 `iterations`가 움직이지 않는다 / `device_graph` opt-in — conditional WHILE, 술어가 `drive`의 루프 헤더 그 자체, instantiate는 **`DevCtx` 전체 memcmp 키로 (deck, shape, 바인딩)당 1회**, 거절은 이름을 남기고 **stream으로 내려간다 — host로가 아니라**); `reigv`가 `DevCtx`에서 device `PprLoopState`로 이동(capture가 굽는 값이므로); 단계 C는 `chif`(`refGeneration`)·`crdf`(쓰기 지점 bump) generation gate + canonical nodal set borrow, **`verify` 모드가 borrow의 건전성을 device에서 bitwise로 대조**; 단계 E는 arena를 만들지 않고 `allocations`/`reallocations`로 **"슬롯당 1회지 statepoint당이 아니다"를 수신증에 세운다**; receipt `schema_version 2` + 14필드; `tools/test_ppr_gpu_contract.py` 20 property·**음성대조 22**; `docs/WP6_PPR_DEVICE_LOOP_20260831_KO.md` / **단계 D(reconstructPinPower 이식) not-started** | 본 작업 | **238 컴파일이 첫 관문**(로컬에 nvcc 없음 — `CudaPprBackend.cu`는 shim 위에서 `CUDART_VERSION` 12060·13000 양쪽 문법 검사만 했다). §6.1 B0가 먼저: PPR OFF vs ON에서 **pin 이외 전 데이터셋 0 차이 + digest `0d15abf29d222a02`/`4382` 동일 + ON×2**, `reallocations == 0`, `allocations == 26`. 그 다음 §6.2 Gate A **0.0000 유지**(NaN 위치 집합이 같을 것 — 다르면 델타가 아니라 실패), §6.3 arm 3종에서 **`iterations`가 셋 다 816**이고 `host_syncs_per_statepoint` 24.3→1.000, PPR phase −25 % 이상. 단계 C는 **`canonical_mismatch == 0`이 먼저**이고 그 다음 `h2d_bytes` −65.1 %(계획 80 % 문턱에는 미달, 이유는 §3.3: `LiveXs`가 PPR까지 오는 경로가 없다). 배치 8×M8+MPS c/h vs **878**, 여기서 PPR ≥ 10 %일 때만 `CudaPprArena`가 근거를 얻는다 | WP2(PPR 1차 A/B), WP1(fail-closed로 engagement 증명). 잔여: XS 블록 676,080 B/sp는 그대로, 계수 7배열 **9,059,472 B/sp D2H도 그대로**(단계 D가 그 소비자를 옮겨야 사라진다) |
