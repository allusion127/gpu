...
def main() -> int:
    ...
    # ---- 9: the residency is scoped to the DRIVE, not to the loop ---------
    #
    # TWO FIXES FOR ONE BUG MET AT THE MERGE.  This branch set the flag from the
    # ARMED decision at loop scope; the campaign branch raises and lowers it
    # around the drive itself.  Loop scope leaves the flag TRUE while the HOST
    # body runs inside an armed loop -- a launch failure, or any
    # `!outer_on_device` pass -- which is the same stale-elision bug one call
    # later, so drive scope won and this checks drive scope.
    #
    # THE INVARIANT: every raise is inside a sweep hook, and every hook that can
    # raise it also lowers it on every exit.  Nothing outside the hooks may set
    # it true at all.
    ...
    _hooks = {
        "outerSweepHook": "static bool outerSweepHook(",
        "outerSweepEnqueueHook": "static bool outerSweepEnqueueHook(",
        "outerSweepFinishHook": "static bool outerSweepFinishHook(",
    }
    _bodies = {name: _fn_body(driver, header) for name, header in _hooks.items()}
    _raises_outside = 0
    for m in re.finditer(r"setOuterSegmentResident\(true\)", driver):
        if not any(b and driver.find(b) <= m.start() < driver.find(b) + len(b)
                   for b in _bodies.values()):
            _raises_outside += 1
    if _raises_outside:
        problems.append(
            "Driver.h: setOuterSegmentResident(true) outside a sweep hook.  The flag means "
            "'the caller of THIS drive owns dhat and psi on the device'; raised anywhere "
            "else it outlives the segment and the sweep elides the dhat and psi H2D while "
            "the HOST body writes the host arrays -- k_eff 1.135 -> 0.678 on kngr_238")
    # The blocking hook raises and lowers within itself; the stream pair raises
    # in the enqueue half and lowers in the finish half AND on its own fallback.
    if _bodies["outerSweepHook"].count("setOuterSegmentResident(false)") < 1:
        problems.append("Driver.h: outerSweepHook raises the residency and never lowers it")
    if _bodies["outerSweepEnqueueHook"].count("setOuterSegmentResident(true)") < 1:
        problems.append("Driver.h: outerSweepEnqueueHook does not raise the residency, so "
                        "stageSweepIO inside enqueueDrive would upload over the buffers the "
                        "segment owns")
    if _bodies["outerSweepEnqueueHook"].count("setOuterSegmentResident(false)") < 1:
        problems.append("Driver.h: the enqueue hook's warm-up fallback takes the whole "
                        "blocking drive and so has no finish half -- it must lower the "
                        "residency itself")
    if _bodies["outerSweepFinishHook"].count("setOuterSegmentResident(false)") < 1:
        problems.append("Driver.h: outerSweepFinishHook does not lower the residency.  It is "
                        "the end of the drive the enqueue half raised it for, and leaving it "
                        "up on a failure path is the sticky-flag bug in a rarer costume")
    if "setOuterSegmentOwnership" in driver:
        problems.append("Driver.h: the loop-scope ownership setter is back beside the "
                        "drive-scope one.  Two mechanisms for one flag means the looser wins "
                        "wherever it sets true first")
    if problems:
